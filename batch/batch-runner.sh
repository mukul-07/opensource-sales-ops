...
# opensource-sales-ops batch runner — standalone orchestrator for claude -p workers
# Reads batch-input.tsv, delegates each prospect to a claude -p worker,
...
usage() {
  cat <<'USAGE'
opensource-sales-ops batch runner — process prospects in batch via claude -p workers
...
main() {
  ...
  echo "=== opensource-sales-ops batch runner ==="
